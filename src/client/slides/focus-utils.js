export const tagNamesShouldFocused = ['P', 'LI', 'TR', 'PRE'];

const MAX_LEVEL_OF_SEARCH_CHILD = 3;

function shouldFocus(element) {
  if (tagNamesShouldFocused.includes(element.tagName)) {
    return true;
  }

  if (element.tagName === 'DIV' && element.children.length === 0 && element.childNodes.length !== 0) {
    return true;
  }
...
function recursiveSearchChildToBeFocused(element, { reverseSearch, level }) {
  if (level >= MAX_LEVEL_OF_SEARCH_CHILD) return null;
  ...
  while (reverseSearch ? i >= 0 : i < element.children.length) {
    ...
    const childToBeFocused = recursiveSearchChildToBeFocused(element.children[i], { reverseSearch, level: level + 1 });
    if (childToBeFocused) {
...
function recursiveSearchSiblingAndUncleToBeFocused(element, { reverseSearch }) {
  ...
  for (;;) {
    ...
    const siblingChildToBeFocused = recursiveSearchChildToBeFocused(sibling, { reverseSearch, level: 0 });
...
function focusOnElement(element, options) {
  ...
  const childToBeFocused = recursiveSearchChildToBeFocused(element, { reverseSearch, level: 0 });
  if (childToBeFocused) {
