...
async function bundleClient(result, struc, production) {
  ...
  for (let i = 0, { length } = result.outputFiles; i < length; i += 1) {
    ...
    if (extname(path) === '.css') {
      filesToBeWritten.push({ path, contents });
      const basenameOfCss = basename(path);
      cssFileNames.push(basenameOfCss);
  ...
  await Promise.all(
    filesToBeWritten.map(async ({ path, contents }) => {
  ...
  const html = createHtml(jsFileName, cssFileNames, production);
